...
public class KenoThread_Offical implements Runnable {
	...
	public KenoThread_Offical(LotteryResultRepository lotteryResultInfoRepository, RestTemplate restTemplate, String url, String lock, RedisService redisService, Integer timeZone) {
		...
		synchronized (lock) {
			try {
				...
				if (openTime.getTime() < dateB.getTime() || openTime.getTime() > System.currentTimeMillis()) {
					logger.info("加拿大28官网拉取{}期开奖数据开奖时间{}错误", term, openTime);
					return;
				}
